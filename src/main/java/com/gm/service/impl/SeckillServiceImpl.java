...
import com.gm.exception.SeckillException;
import com.gm.service.SeckillService;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public Product getById(long seckillId) {
        return productDao.queryById(seckillId);
    }
    ...
    public Exposer exportSeckillUrl(long seckillId) {
        ...
        Product product = redisDao.getSeckill(seckillId);
        if (product == null) {
            //2.访问数据库
            product = productDao.queryById(seckillId);
    ...
    public SeckillExecution executeSeckill(long seckillId, long userPhone, String md5)
        ...
        try {
            ...
            //否则更新了库存，秒杀成功,增加明细
            int insertCount = orderDao.insertOrder(seckillId, userPhone);
            ...
            if (insertCount <= 0) {
                ...
                //减库存,热点商品竞争
                int updateCount = productDao.reduceNumber(seckillId, nowTime);
                if (updateCount <= 0) {
                    //没有更新库存记录，说明秒杀结束 rollback
                    throw new SeckillCloseException("seckill is closed");
                } else {
                    //秒杀成功,得到成功插入的明细记录,并返回成功秒杀的信息 commit
                    Order order = orderDao.queryByProductId(seckillId, userPhone);
    ...
    public SeckillExecution executeSeckillProcedure(long seckillId, long userPhone, String md5) {
        ...
        try {
            ...
            if (result == 1) {
                Order sk = orderDao.queryByProductId(seckillId, userPhone);
